...
class ViewController: UIViewController {
  ...
  // MARK: - Selectors
    ...
    enemyView.bounds.size = CGSize(width: 15, height: 15)
    enemyView.backgroundColor = getRandomColor()
    let urlEnemy : String = "https://media1.tenor.com/m/GOabrbLMl4AAAAAd/plink-cat-plink.gif"
    let imageUrlEnemy = UIImage.gifImageWithURL(urlEnemy)
    enemyView.image = imageUrlEnemy
...
fileprivate extension ViewController {
    func setupPlayerView() {
        let url : String = "https://media1.tenor.com/m/VU-02rOt8Y4AAAAd/corgi-smirk.gif"
        let imageURL = UIImage.gifImageWithURL(url)
        playerView.backgroundColor = .systemBlue
        playerView.image = imageURL
